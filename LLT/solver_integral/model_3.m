function status_d = model_3(t,status,parameters)
...
C_d = 0.000508 * alpha^2 + 0.004228 * alpha + 0.0161;
...
status_d(1) = status(4) * cos(status(5)) * cos(status(6));
status_d(2) = status(4) * cos(status(5)) * sin(status(6));
status_d(3) = status(4) * sin(status(5));
status_d(4) = -D/m - g*sin(status(5));
status_d(5) = L*cos(sigma)/(m*status(4)) - g* cos(status(5))/status(4);
status_d(6) = L*sin(sigma)/(m*status(4)*cos(status(5)));